...
if %errorlevel% == 0 (
    ...
    REM Find the output SVG file
    REM If user specified output, use that, otherwise find the auto-generated one
    if not "%~2"=="" (
        set "SVG_FILE=%~2"
    ) else (
        REM Auto-generated filename is input_bends.svg
        for %%f in ("%~1") do set "SVG_FILE=%%~dpnf_bends.svg"
    )
    
    echo Looking for SVG file: !SVG_FILE!
    
    REM Check if file exists before trying to open
    if exist "!SVG_FILE!" (
        echo Opening SVG in browser...
        start "" "!SVG_FILE!"
    ) else (
        echo WARNING: SVG file not found: !SVG_FILE!
        echo The file may have been created with a different name.
        pause
    )
) else (
    echo.

...
# Pipe SQL to sqlite3 instead of passing as argument
$schemaSQL | & $sqliteExe $dbPath
...
if (Test-Path $csvPath) {
    ...
    Write-Host "  Copy Material_List.csv to D:\MRP_System\ and re-run" -ForegroundColor Yellow
}
...
$count = "SELECT COUNT(*) FROM raw_materials;" | & $sqliteExe $dbPath
Write-Host "  Materials in database: $count" -ForegroundColor Green

".tables" | & $sqliteExe $dbPath
...
Write-Host "=============================================" -ForegroundColor Cyan
Write-Host ""
Write-Host "Restart MRP-API-Server: nssm restart MRP-API-Server" -ForegroundColor Yellow

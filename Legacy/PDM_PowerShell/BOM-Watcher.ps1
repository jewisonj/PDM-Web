...
# Check and add cut_time column
$hasCutTime = & $Global:SQLiteExe $Global:DBPath "PRAGMA table_info(items);" 2>$null | Select-String "cut_time"
if (-not $hasCutTime) {
    Exec-SQL "ALTER TABLE items ADD COLUMN cut_time REAL;"
    Write-Log "Added cut_time column to items table"
}

# Check and add price_est column
$hasPriceEst = & $Global:SQLiteExe $Global:DBPath "PRAGMA table_info(items);" 2>$null | Select-String "price_est"
if (-not $hasPriceEst) {
    Exec-SQL "ALTER TABLE items ADD COLUMN price_est REAL;"
    Write-Log "Added price_est column to items table"
}
...
Register-ObjectEvent $watcher Created -Action {
    ...
    for ($i = 0; $i -lt $lines.Count; $i++) {
        if ($lines[$i] -match 'Model Name') {
            ...
            # DESCRIPTION, PROJECT, PRO_MP_MASS, PTC_MASTER_MATERIAL, CUT_LENGTH, SMT_THICKNESS, CUT_TIME, PRICE_EST
    ...
    foreach ($childItem in $children.Keys) {
        ...
        $cutTime = if ($null -ne $childData.CutTime) { $childData.CutTime } else { "NULL" }
        $priceEst = if ($null -ne $childData.PriceEst) { $childData.PriceEst } else { "NULL" }
        
        if ($childData.IsSupplier) {
            # Supplier part - update with supplier-specific fields
            $suppPrefix = $childData.SupplierPrefix
            $suppPN = $childData.SupplierPN.Replace("'", "''")
            Exec-SQL "UPDATE items SET description='$desc', project='$proj', material='$mat', mass=$mass, thickness=$thick, cut_length=$cutLen, cut_time=$cutTime, price_est=$priceEst, is_supplier_part=1, supplier_prefix='$suppPrefix', supplier_pn='$suppPN', modified_at=CURRENT_TIMESTAMP WHERE item_number='$childItem';"
        }
        else {
            # Internal part - standard update
            Exec-SQL "UPDATE items SET description='$desc', project='$proj', material='$mat', mass=$mass, thickness=$thick, cut_length=$cutLen, cut_time=$cutTime, price_est=$priceEst, modified_at=CURRENT_TIMESTAMP WHERE item_number='$childItem';"
        }
